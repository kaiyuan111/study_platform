...
<div class='form'>
	<form method='post' action='/main/action/edit'>
		...
		<br>
		是否菜单:
		<input type='checkbox' name='is_menu'  <?php echo !empty($entity['is_menu']) ? 'checked' : ''; ?>
		/>
		<br> <br>
		<div class="logo_paths_choose">
			...
			<ul class='normal_table'>
				<li>
					<input type='checkbox' name='logopath' value='<?php echo $path['unionpath'];?>
					'
					<?php echo !empty($entity['logo'])&&$path['path']==$entity['logo'] ? 'checked' : ''; ?>
					/>
					<img src='<?php echo $path['path'];?>
					'/>
					<img src='<?php echo $path['clickpath'];?>'/></li>
			</ul>
			<?php } ?>
		</div>
